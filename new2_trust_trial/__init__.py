...
class Player(BasePlayer):
    ...
    player_name = models.StringField(label='お名前を入力してください')
    sent_amount = models.CurrencyField(
        min=0, max=C.ENDOWMENT,
        label='送金額を入力してください（0〜10）',
    )
    received_amount = models.CurrencyField()        # 振り分けられた金額
    sent_back_amount = models.CurrencyField(
        min=cu(0),
        label='返送額を入力してください',
    )
    matched_sender_id = models.StringField()        # 誰の送金を受けたか（ID）

    # ----------------------------------------
    # アンケートA：A役での送信理由
    # ----------------------------------------

    # A-Q1：相手がどの程度返してくれると思ったか（0〜100%）
    ...
    # B-Q1：一般的リスク態度（0〜10スケール）

    # B-Q2：婉曲的リスク選好（0〜10ポイントの投資額）
