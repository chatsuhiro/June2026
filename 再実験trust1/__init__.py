...
# 1. 定員に入れた人に最初に見せるページ
class Wait(Page):
    template_name = '再実験trust1/Wait.html'


# 2. 定員が全員ここに揃うまで全員を通さないストッパー（待機画面）
class EntryLock(WaitPage):
    # template_name は消して、oTree標準の待機画面を使います

    @staticmethod
    def wait_for_all_groups(subsession):
        return True  # ★グループ関係なく、この実験(サブセッション)の全員が揃うまで待つ設定

    # 参加者全員がこの待機画面に到達するまで、次の「名前入力」へ進ませない
...
page_sequence = [Wait, EntryLock, Page0, Page1, Page2, Page3, Page4, Page5, Page6, Page7]
